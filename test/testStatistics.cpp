...
TEST(StatisticsTest, Timestamps) {
  ...
  getProtocols("./pcaps/http-jpeg.pcap", protocols, state, [&](pfwl_status_t, pfwl_dissection_info_t r) {
    if (last_timestamp && check_timestamp && r.l4.direction == direction) {
      EXPECT_TRUE(r.flow_info.statistics[PFWL_STAT_TIMESTAMP_LAST][direction] -
                          r.flow_info.statistics[PFWL_STAT_TIMESTAMP_FIRST][direction] ==
                      2 ||
                  r.flow_info.statistics[PFWL_STAT_TIMESTAMP_LAST][direction] -
                          r.flow_info.statistics[PFWL_STAT_TIMESTAMP_FIRST][direction] ==
                      3);
