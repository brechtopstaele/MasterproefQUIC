...
=======
const double expected_ts[2][10] = {{0},
                                   {0.000000, 130187.000000, 630907.000000, 110159.000000, 80115.000000, 260374.000000,
                                    480691.000000, 120173.000000, 140202.000000, 470677.000000}};
const double expected_lengths[2][10] = {{479}, {1380, 1380, 1380, 1380, 1380, 1380, 1380, 1380, 1380, 1380}};
...
=======
  pfwl_state_t *state = pfwl_init();
  pfwl_set_flow_termination_callback(state, &splt);
  pfwl_set_timestamp_unit(
      state, PFWL_TIMESTAMP_UNIT_MICROSECONDS); // Since we set PCAP timestamps (which are expressed as microseconds).
  std::vector<uint> protocols;
  targetflow = 0;
  getProtocols(
      "./pcaps/http.cap", protocols, state, [&](pfwl_status_t, pfwl_dissection_info_t) { ; }, true);
>>>>>>> SoftAtHome/master
