...
TEST(HTTPTest, ContentType) {
  ...
  //  unsigned char first_bytes[] = {0xd8, 0xff, 0xe0, 0xff, 0x10, 0x00, 0x46, 0x4a, 0x46,  0x49, 0x01, 0x00, 0x01,
  //  0x01, 0x48, 0x00}; unsigned char last_bytes[] = {0x6c, 0x7d, 0xf7, 0xfa, 0x9b, 0xd9, 0x53, 0xfd, 0x9f, 0xea, 0xfc,
  //  0x73, 0xec, 0x8f, 0x36, 0xf7, 0x59, 0x7f, 0xff, 0x9f, 0xd9};
  unsigned char first_bytes[] = {0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 0x4a, 0x46,
                                 0x49, 0x46, 0x00, 0x01, 0x01, 0x01, 0x00, 0x48};
  unsigned char last_bytes[] = {0x7d, 0x6c, 0xfa, 0xf7, 0xd9, 0x9b, 0xfd, 0x53, 0xea, 0x9f, 0x73,
                                0xfc, 0x8f, 0xec, 0xf7, 0x36, 0x7f, 0x59, 0x9f, 0xff, 0xd9};
  std::vector<uint> protocols;
  pfwl_state_t *state = pfwl_init();
    ...
    getProtocols(filename, protocols, state, [&](pfwl_status_t, pfwl_dissection_info_t r) {
      if (r.l7.protocol == PFWL_PROTO_L7_HTTP) {
        ...
        if (!pfwl_http_get_header(&r, "User-Agent", &field)) {
          uAgentFound = true;
          EXPECT_FALSE(strncmp((const char *) field.value,
...
TEST(HTTPTest, Tags) {
  ...
  pfwl_field_string_tags_add_L7(state, PFWL_FIELDS_L7_HTTP_BODY, "<?xml version", PFWL_FIELD_MATCHING_PREFIX,
                                "TAG_PREFIX");
  pfwl_field_mmap_tags_add_L7(state, PFWL_FIELDS_L7_HTTP_HEADERS, "user-agent", "mozilla", PFWL_FIELD_MATCHING_PREFIX,
                              "TAG_MOZILLA");
  pfwl_field_mmap_tags_add_L7(state, PFWL_FIELDS_L7_HTTP_HEADERS, "host", "www.ethereal", PFWL_FIELD_MATCHING_PREFIX,
                              "TAG_ETHEREAL");
  ...
  getProtocols("./pcaps/http.cap", protocols, state, [&](pfwl_status_t, pfwl_dissection_info_t r) {
    for (size_t i = 0; i < r.l7.tags_num; i++) {
      ...
      if (r.l7.tags_num && !strcmp(r.l7.tags[i], "TAG_PREFIX")) {
        foundPrefix = true;
      }

      if (r.l7.tags_num && !strcmp(r.l7.tags[i], "TAG_MOZILLA")) {
        foundMozilla = true;
      }

      if (r.l7.tags_num && !strcmp(r.l7.tags[i], "TAG_ETHEREAL")) {
        foundEthereal = true;
      }
    }
  });
