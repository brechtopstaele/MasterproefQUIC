...
TEST(HTTPTest, ContentType) {
  ...
  getProtocolsCpp("./pcaps/http-jpeg.pcap", protocols, &state, [&](peafowl::Status, peafowl::DissectionInfo &r) {
    if (r.getL7().getProtocol() == PFWL_PROTO_L7_HTTP) {
      if ((*r.getFlowInfo().getUserData() == NULL) && (field = r.getL7().httpGetHeader("Content-Type")).isPresent() &&
          field.getString() == "image/jpeg") {
        ctFound = true;
        r.getFlowInfo().setUserData((void *) 1);
      }

      if ((field = r.getL7().getField(PFWL_FIELDS_L7_HTTP_BODY)).isPresent() && *r.getFlowInfo().getUserData()) {
        bodyFound = true;
    ...
    getProtocolsCpp(filename, protocols, &state, [&](peafowl::Status, peafowl::DissectionInfo &r) {
      if (r.getL7().getProtocol() == PFWL_PROTO_L7_HTTP) {
        ...
        if ((field = r.getL7().httpGetHeader("User-Agent")).isPresent()) {
          uAgentFound = true;
          EXPECT_STREQ(field.getString().c_str(),
...
TEST(HTTPTest, Tags) {
  ...
  state.fieldMmapTagsAddL7(PFWL_FIELDS_L7_HTTP_HEADERS, "user-agent", "mozilla", PFWL_FIELD_MATCHING_PREFIX,
                           "TAG_MOZILLA");
  state.fieldMmapTagsAddL7(PFWL_FIELDS_L7_HTTP_HEADERS, "host", "www.ethereal", PFWL_FIELD_MATCHING_PREFIX,
                           "TAG_ETHEREAL");
