...
void getProtocols(
    const char *pcapName, std::vector<uint> &protocols, pfwl_state_t *state = NULL,
    std::function<void(pfwl_status_t, pfwl_dissection_info_t)> lambda = [](pfwl_status_t, pfwl_dissection_info_t) {},
    bool pcap_ts = false);
void getProtocolsCpp(
    const char *pcapName, std::vector<uint> &protocols, peafowl::Peafowl *state = NULL,
    std::function<void(peafowl::Status, peafowl::DissectionInfo &)> lambda = [](peafowl::Status,
                                                                                peafowl::DissectionInfo &) {},
    bool pcap_ts = false);
>>>>>>> SoftAtHome/master
